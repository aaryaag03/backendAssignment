            ...
            if (result[0] != undefined && result[0].password === hash) {
                ...
                (error, result1, fields)=>{
                    if(error){
                        ...
                        db.query("SELECT * from r where username="+db.escape(session.userid)+";",
                        (error ,result3, fields)=>{
                        db.query("SELECT * from books where username="+db.escape(session.userid)+";",
                        (error ,result2, fields)=>{
                        return res.render('clientLoggedin', { data: result[0].fees , book_list: result1 , my_books:result2 , my_requests:result3});
                        });
                    });
                    }
                });
        ...
        if (result[0]===undefined ) {
            if(req.body.username && (req.body.password==req.body.password1)){
                if(req.body.password.length<4){
                    res.send('MIN PASSWORD LENGTH IS 4');
                }
                else{
                ...
                (error, result1, fields)=>{
                    if(error){
                        ...
                        return res.render('clientLoggedin', {  data: result4[0].fees  , book_list: result1 , my_books:result2, my_requests:result3});
                    });
                        });
                    });
                    }
...
router.post('/addBook',(req,res)=>{  
    ...
    (error,result,fields)=>{
        if(result[0]===undefined){
            ...
            res.send('BOOK HAS ALREADY BEEN ADDED.')
        }
    });
                ...
                (error, result, fields)=>{
                    if(error){
                        ...
                        (error, result,fields)=>{
                            ...
                            console.log(t2);
                        if(t2-t1 >60000){
                            db.query("select * from client where username="+db.escape(session.userid)+";",
                            (error,result1,fields)=>{
                                let fee= result1[0].fees +10;
                            db.query("update client set fees="+db.escape(fee)+" where username="+db.escape(session.userid)+";"); 
                            });
                        }
                        return res.send('REQUEST SENT');
